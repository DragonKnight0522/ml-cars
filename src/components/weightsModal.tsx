...
const WeightsModal = (props: { show: boolean; handleHide: () => void }) => {
			...
			<tbody>
				{props.weights.map((row, i) => {
					return (
						<tr key={i + "weightRow"}>
							{row.map((weight, j) => {
								return <td key={j + "weight"}>{weight}</td>;
							})}
						</tr>
					);
				})}
