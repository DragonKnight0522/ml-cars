...
const NetworkCanvas = (props: { network: Network }) => {
	...
	const drawLevel = (
		...
		for (let i = 0; i < inputs.length; i++) {
			for (let j = 0; j < outputs.length; j++) {
				ctx.beginPath();
				ctx.moveTo(left, getNodeY(inputs, i, bottom, top));
				ctx.lineTo(right, getNodeY(outputs, j, bottom, top));
				ctx.lineWidth = 2;
	...
	return (
		<canvas
			...
			height={NetworkCanvasDefaultHeight}
		/>
	);
};
