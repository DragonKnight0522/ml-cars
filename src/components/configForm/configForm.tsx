...
const Modal = (props: { open: boolean; close: () => void }) => {
	...
	const onSubmit = (data: any) => {
		...
		const config =
			appContext.activeConfig.name !== data.name ||
		...
		if (config.layers.length === data.layers.length) {
			for (let i = 0; i < data.layers.length; i++) {
				...
				if (
					layer.outputs === config.layers[i].outputs &&
					layer.inputs === config.layers[i].inputs
				) {
