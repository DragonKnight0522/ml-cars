...
export class Simulator {
	...
	protected updateSmartCars() {
		...
		this.smartCars = this.smartCars.filter(
			(car) => !toDelete.includes(car.id) || car.id === bestCar.id
		);
	}
	...
	protected generateSmartCars() {
		...
		for (let i = 0; i < this.brainCount; i++) {
			...
			if (i > 0)
				car.brain = smartCars[0].brain.mutate(
					this.brainConfig.mutationAmount,
					this.brainConfig.mutationRate
				);
			smartCars.push(car);
		}
		return smartCars;
...
export class Teacher extends Simulator {
	...
	protected generateSmartCars() {
		const y = this.road.getLaneCenter(this.startLane);
		return [
			new SmartCar(0, 0, y, this.driverSpeed, this.brainConfig, true),
		];
	}
